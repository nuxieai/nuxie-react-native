...
@objc(NuxieBridge)
public final class NuxieBridge: NSObject {
  @objc public var emit: ((String) -> Void)?
  private var session: String?
  private var snapshotSubscription: AnyCancellable?
  private static weak var owner: NuxieBridge?
      ...
      do {
        if method != "configure" {
          guard Self.owner === self, let session = self.session, arguments["session"] as? String == session else {
            throw failure("sessionExpired", "Configure Nuxie in this JavaScript runtime before calling its API")
          }
        }
        ...
        case "configure":
          ...
          if sdk.isSetup && Self.configurationKey == nil { throw failure("alreadyConfigured", "Nuxie was configured outside React Native") }
          Self.owner = self
...
#if DEBUG
          if let endpoint = ProcessInfo.processInfo.environment["NUXIE_RN_API_ENDPOINT"], let url = URL(string: endpoint) {
        ...
        case "identify":
          ...
          resolve(nil)
        case "reset": sdk.reset(keepAnonymousId: arguments["keepAnonymousId"] as? Bool ?? false); resolve(nil)
        ...
        case "consumeFeature":
          ...
          resolve(try encode(["operationId": result.operationId, "accepted": result.accepted, "code": result.code,
...
private func failure(_ code: String, _ message: String) -> NSError {
  NSError(domain: "ai.nuxie.reactnative", code: 1, userInfo: ["code": code, NSLocalizedDescriptionKey: message])
}
