...
All four combinations passed the shared Lab checks: identified customer, explicit entity grant, denied unknown entity, committed consumption, idempotent same-ID replay, exactly one balance decrement, unchanged second entity, and anonymous reset. Expo receipts include `rn-ios-1789314641067` and `rn-android-1789314666569`. These are backend receipts from actual native calls, not mocked bridge responses.

Both Android release APKs passed `zipalign -c -P 16 -v 4` for 16 KiB ZIP page alignment. ELF program-header inspection also passed: all load segments in the 26 bare-host and 32 Expo-host 64-bit shared libraries had alignment of at least 16 KiB.
...
- Resolve iOS live Feature readiness after reset/reidentify and bind the final consumption/entity native commits from [UNIV-3135](https://universe.basis.dev/issue/UNIV-3135).
- Repeat final-pin native builds and clean Expo prebuild.
- Verify the authored Experience renders through the local release authority.
